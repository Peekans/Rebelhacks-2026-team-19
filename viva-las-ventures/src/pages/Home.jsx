...
>>>>>>> Stashed changes
  ...
  {
    title: 'AI Concierge',
    description: 'Chat with our AI assistant for personalized Vegas recommendations and tips.',
    icon: (
      <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path
          strokeLinecap="round"
...
>>>>>>> Stashed changes
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-10 w-full">
            {ACTION_CARDS.map((card, i) => (
              ...
              >
                ...
                <h3 className="text-xl font-heading font-semibold text-white mb-3">{card.title}</h3>
                <p className="text-white/60 font-body leading-relaxed text-sm">{card.description}</p>
      ...
      <footer className="border-t border-white/5 py-8 px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Logo size={28} />
            <span className="text-sm text-white/40 font-body">Viva Las Ventures</span>
          </div>
          <p className="text-xs text-white/30 font-body">Built with React, Firebase, Tailwind CSS &amp; Claude AI</p>
        </div>
      </footer>
    </div>
  )
